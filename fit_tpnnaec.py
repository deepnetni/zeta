...
from utils.trunk import AECTrunk, pad_to_longest_aec
...
def vad_detect(s):
    vad_detect = VAD(blk_ms=10)
    out = vad_detect.vad_frames(s)
    # out = vad_detect.vad_waves(s)
    # audiowrite("test.wav", np.stack([s, out], axis=-1), 16000)
    return out
...
class Train(Engine):
    def __init__(
        ...
        **kwargs,
        ...
        self.stft = STFT(nframe=320, nhop=160, nfft=320).to(self.device)
        self.stft.eval()
    ...
    #     )
    def _config_optimizer(self, name: str, params, **kwargs):
        alpha = kwargs.get("alpha", 1.0)
        supported = {
            "adam": lambda p: torch.optim.Adam(p, lr=alpha * 1e-3, amsgrad=False),
            "adamw": lambda p: torch.optim.AdamW(p, lr=alpha * 1e-3, amsgrad=False),
            "rmsprop": lambda p: torch.optim.RMSprop(p, lr=alpha * 1e-3),
        }
        return supported[name](params)

    def loss_fn_1(self, clean: Tensor, enh: Tensor, enh_c, vad_p, nlen=None) -> Dict:
        specs_enh = self.stft.transform(enh)  # b,2,t,f
        specs_enh_ = self.stft.transform(enh_c)  # b,2,t,f
        specs_sph = self.stft.transform(clean)

        mag_enh = specs_enh.pow(2).sum(1).sqrt()
        mag_enh_ = specs_enh_.pow(2).sum(1).sqrt()
        mag_sph = specs_sph.pow(2).sum(1).sqrt()

        mag_loss = F.l1_loss(mag_enh, mag_sph)
        r_loss = F.l1_loss(specs_enh[:, 0, ...], specs_sph[:, 0, ...])
        i_loss = F.l1_loss(specs_enh[:, 1, ...], specs_sph[:, 1, ...])

        mag_loss_ = F.l1_loss(mag_enh_, mag_sph)
        r_loss_ = F.l1_loss(specs_enh_[:, 0, ...], specs_sph[:, 0, ...])
        i_loss_ = F.l1_loss(specs_enh_[:, 1, ...], specs_sph[:, 1, ...])

        loss_c = mag_loss_ + r_loss_ + i_loss_
        loss_f = mag_loss + r_loss + i_loss

        vad_l = np.array(Parallel(n_jobs=10)(delayed(vad_detect)(s) for s in clean.cpu().numpy()))
        vad_lbl = torch.tensor(vad_l, dtype=torch.long).to(clean.device)  # B,T
        # vad_lbl = torch.tensor(vad_l).to(clean.device)  # B,T
        vad_p = vad_p[:, : vad_lbl.shape[-1], :]  # B,T,2
        loss_vad = F.cross_entropy(vad_p.reshape(-1, 2), vad_lbl.reshape(-1))

        loss = 0.3 * loss_c + 0.7 * loss_f + 0.06 * loss_vad

        loss_dict = {
            "loss_c": 0.3 * loss_c.cpu().detach().item(),
            "loss_f": 0.7 * loss_f.cpu().detach().item(),
            "loss_vad": 0.06 * loss_vad.cpu().detach().item(),
            "loss": loss,
        }

        if torch.isnan(loss).any():
            raise RuntimeError(f"\nNan:{loss_dict}")
        # self.hist_loss = loss_dict
    ...
    def _fit_each_epoch(self, epoch):
        ...
        for mic, ref, sph in pbar:
            ...
            ref = ref.to(self.device)  # B,T
            sph = sph.to(self.device)  # B,T

            self.optimizer.zero_grad()
            enh, enh_c, vad_p = self.net(mic, ref)
            loss_dict = self.loss_fn_1(sph[:, : enh.size(-1)], enh, enh_c, vad_p)

            loss = loss_dict["loss"]
            loss.backward()

            self.check_grads(loss_dict)
    ...
    def _valid_dsets(self):
        ...
        for mic, _, sph, nlen in pbar:
            mic = mic.to(self.device)  # B,T,6
            # ref = ref.to(self.device)  # B,T,6
            sph = sph.to(self.device)  # b,c,t,f
            nlen = self.stft.nLen(nlen).to(self.device)  # B,

            metric_dict = self.valid_fn(sph, mic, mic, None, nlen, return_loss=False)
    ...
    def valid_fn(
        self, sph: Tensor, enh: Tensor, enh_c, vad_p, nlen_list: Tensor, return_loss: bool = True
    ) -> Dict:
        """
        B,T
        """
        ...
        if return_loss:
            loss_dict = self.loss_fn_1(sph[..., : enh.size(-1)], enh, enh_c, vad_p, nlen_list)
        else:
            loss_dict = {}
    ...
    def _valid_each_epoch(self, epoch):
        ...
        for mic, ref, sph, nlen in pbar:
            ...
            with torch.no_grad():
                enh, enh_c, vad_p = self.net(mic, ref)  # B,T,M

            metric_dict = self.valid_fn(sph, enh, enh_c, vad_p, nlen)
    ...
    def _vtest_each_epoch(self, epoch):
        ...
        for mic, ref, sph, nlen in pbar:
            ...
            metric_dict = self.valid_fn(sph, enh, enh_c, vad_p, nlen, return_loss=False)
            # record the loss
            metric_rec.update(metric_dict)
            # pbar.set_postfix(**metric_rec.state_dict())
            # break
...
def parse():
    ...
    parser.add_argument("--conf", help="config file", default="config/conf_tpnn.yaml")
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        for mic, ref, _, fname in tqdm(dset):
            mic = mic.cuda()
            ref = ref.cuda()
            with torch.no_grad():
                out, _, _ = net(mic, ref)
