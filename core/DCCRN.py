...
@tables.register("models", "dccrn")
class DCCRN(nn.Module):
    """DCCRN: Deep Complex Convolution Recurrent Network for Phase-Aware Speech Enhancement
    https://github.com/huyanxin/DeepComplexCRN/blob/master/dc_crn.py
    """
    ...
    def __init__(
        ...
        self.istft = ConviSTFT(
            self.win_len, self.win_inc, fft_len, self.win_type, "complex", fix=fix
        )
    ...
    def loss(self, labels, inputs, loss_mode="SI-SNR"):
        if loss_mode == "MSE":
            b, d, t = inputs.shape
            labels[:, 0, :] = 0
            labels[:, d // 2, :] = 0
            ...
            return dict(loss=-si_snr(inputs, labels))
        elif loss_mode == "MAE":
...
@tables.register("models", "dccrn_fig6")
class DCCRN_fig6(nn.Module):
    """DCCRN: Deep Complex Convolution Recurrent Network for Phase-Aware Speech Enhancement
    https://github.com/huyanxin/DeepComplexCRN/blob/master/dc_crn.py
    """
    ...
    def __init__(
        ...
        super(DCCRN_fig6, self).__init__()
    ...
    def forward(self, inputs, HL):
        specs = self.stft(inputs)  # b,2f,t
        hl = expand_HT(HL, specs.shape[-1], self.reso)  # B,C(1),T,F
        hl = hl.transpose(-1, -2).squeeze(1)  # b,t,f
        ...
        spec_phase = torch.atan2(imag, real)
        spec_phase = spec_phase
        cspecs = torch.stack([real, hl, imag, hl], 1)
        cspecs = cspecs[:, :, 1:]
        """
        means = torch.mean(cspecs, [1,2,3], keepdim=True)
        std = torch.std(cspecs, [1,2,3], keepdim=True )
...
if __name__ == "__main__":
    ...
    inputs = torch.randn([1, 16000]).clamp_(-1, 1)
    hl = torch.randn([1, 6]).clamp_(-1, 1)
    labels = torch.randn([1, 16000]).clamp_(-1, 1)
    ...
    outputs = net(inputs, hl)[1]
    check_flops(net, inputs, hl)
