...
class Model_cfg:
    ...
    num_tfxlstm: int = 4  # Number of Time-Frequency xLSTM (TFxLSTM) blocks in the model.
    embed_dim: int = 64  # Inner dimension in xLSTM blocks.
    expansion_factor: int = 4  # Expansion factor for the layers within the xLSTM blocks.
...
class VitPatchEmbed(nn.Module):
    def __init__(
        self, dim, num_channels, resolution, patch_size, stride=None, init_weights="xavier_uniform"
    ):
        super().__init__()
...
# Modified from https://github.com/NX-AI/vision-lstm/blob/main/vision_lstm/vision_lstm2.py
# All rights reserved with the original authors
...
class LinearHeadwiseExpand(nn.Module):
    ...
    def extra_repr(self):
        return f"dim={self.dim}, " f"num_heads={self.num_heads}, " f"bias={self.bias is not None}, "
...
class xLSTMSENet(nn.Module):
    ...
    def __init__(self, cfg=init_cfg):
        """
        ...
        self.cfg = cfg
        self.num_tscblocks = (
            cfg["model_cfg"]["num_tfxlstm"] if cfg["model_cfg"]["num_tfxlstm"] is not None else 4
        )  # default tfmamba: 4
