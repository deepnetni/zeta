...
ERB_fb = np.load("./mtfaa_erb.npy").astype(np.float32)
...
class AttentionMask(nn.Module):
    ...
    def forward(self, inp):
        ...
        if self.causal:
            ...
            masked = torch.fill_(
                torch.empty([batch_size, max_seq_len, max_seq_len]), self.mask_value
            )
...
class PE(nn.Module):
    def __init__(self):
        ...
        self.complex_conv = ComplexConv2d(
            in_channels=1, out_channels=4, kernel_size=(3, 1), stride=(1, 1)
        )
...
class TFCM_block(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = torch.nn.functional.pad(
            x1, pad=(1, 1, int(self.dilation * (self.k_t - 1)), 0, 0, 0), mode="constant", value=0
        )
...
class TFCM(nn.Module):
    def __init__(self, B, in_channels):
        ...
        self.blocks = nn.ModuleList(
            [TFCM_block(in_channels, (3, 3), (2**b, 1)) for b in range(B)]
        )
...
class ASA(nn.Module):
    ...
    def forward(self, x):
        ...
        local_mask = local_mask.view(
            local_mask.shape[0], 1, local_mask.shape[2], local_mask.shape[2]
        )  # shape = (Bs,1, F, F)
...
class FU(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.pointwise_3 = nn.ConvTranspose2d(
            in_channels, out_channels, kernel_size=(1, 7), stride=(1, 4), groups=2
        )
    ...
    def __init__(self, in_channel, filter_size, kernel_size, strides, paddings, **kwargs):
        ...
        for i in range(self.N_layer):
            if i == 0:
                ...
                self.conv_list.append(
                    nn.Conv2d(
                        in_channels=self.filter_size[i - 1],
                        out_channels=self.filter_size[i],
                        kernel_size=self.kernel_size[i],
                        stride=self.strides[i],
                    )
...
class MTFAA_Net(nn.Module):
    def __init__(self):
        ...
        self.pe = PE()
        self.bm = BM(fft_num=512, f_c=256)
