...
def wer(clean_speech, denoised_speech):
    ...
    def _transcription(clean_speech, denoised_speech):
        ...
        # input_values = processor(clean_speech, return_tensors="pt").input_values
        ...
        logits = wer_model(input_values).logits
        # input_values = processor(denoised_speech, return_tensors="pt").input_values
        # logits = model(input_values).logits
        predicted_ids = torch.argmax(logits, dim=-1)
        transcript_estimate = wer_tokenizer.batch_decode(predicted_ids)[0]
        # transcript_estimate = processor.batch_decode(predicted_ids)[0]

        del wer_tokenizer, wer_model
...
def wer_cuda(clean_speech, denoised_speech):
    """
    computes the word error rate(WER) score for 1 single data point
    """
    ...
    def _transcription(clean_speech, denoised_speech):
        ...
        # transcribe clean audio
        input_values = wer_tokenizer(clean_speech, return_tensors="pt").input_values
        logits = wer_model(input_values.cuda()).logits
...
def task1_metric(clean_speech, denoised_speech, sr=16000):
    ...
    """
    WER = wer_cuda(clean_speech, denoised_speech)
...
# TASK 2 METRICS
def location_sensitive_detection(
    pred, true, n_frames=100, spatial_threshold=2.0, from_csv=False, verbose=False
):
    """
        ...
        if num_true_items == 0:  # if there are PREDICTED but not TRUE events
            ...
            for i_t in range(len(t)):  # iterate all true events
                ...
                for i_p in range(len(p)):  # compare each true event with all predicted events
                    ...
                    spat_error = np.linalg.norm(
                        true_coord - pred_coord
                    )  # cartesian distance between spatial coords
                    if (
                        true_class == pred_class and spat_error < spatial_threshold
            ...
            matched = min(
                len(unique_ids), len(unique_ids_t)
            )  # compute the number of actual matches without duplicates
    ...
    results = {"precision": precision, "recall": recall, "F score": F_score}
...
def compute_seld_metrics(predicted_folder, truth_folder, n_frames=100, spatial_threshold=0.3):
    ...
    for i in range(n_files):
        ...
        tp, fp, fn = location_sensitive_detection(
            predicted_temp_path, truth_temp_path, n_frames, spatial_threshold
        )
        TP += tp
        FP += fp
        FN += fn
