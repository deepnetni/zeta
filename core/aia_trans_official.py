...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # src_norm = src
        src2 = self.self_attn(
            src_norm, src_norm, src_norm, attn_mask=src_mask, key_padding_mask=src_key_padding_mask
        )[0]
...
class dense_encoder(nn.Module):
    def __init__(self, in_channel=2, width=64, nbin=257):
        ...
        self.inp_norm = nn.LayerNorm(nbin)
        self.inp_prelu = nn.PReLU(self.width)
        self.enc_dense1 = DenseBlock(nbin, 4, self.width)  # [b, 64, nframes, 512]
        self.enc_conv1 = nn.Conv2d(
            in_channels=self.width, out_channels=self.width, kernel_size=(1, 3), stride=(1, 2)
        )  # [b, 64, nframes, 256]
        self.enc_norm1 = nn.LayerNorm(nbin // 2)
...
class dense_encoder_mag(nn.Module):
    def __init__(self, in_channel=1, width=64, nbin=257):
...
class dense_decoder(nn.Module):
    def __init__(self, width=64, nbin=257):
...
class dense_decoder_masking(nn.Module):
    def __init__(self, width=64, nbin=257):
        ...
        self.mask1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(1, 1)), nn.Sigmoid()
        )
...
class aia_complex_trans_mag(nn.Module):
    ...
    def forward(self, x):
        ...
        x_r_out, x_i_out = (x_mag_out * torch.cos(x_phase_ori)), (
            x_mag_out * torch.sin(x_phase_ori)
        )
...
@tables.register("models", "DB-AIAT")
class dual_aia_trans_merge_crm(nn.Module):
    def __init__(self, nframe=512, nhop=256):
        ...
        self.en_ri = dense_encoder(nbin=self.nbin)
        self.en_mag = dense_encoder_mag(nbin=self.nbin)
        ...
        self.de1 = dense_decoder(nbin=self.nbin)
        self.de2 = dense_decoder(nbin=self.nbin)
        self.de_mag_mask = dense_decoder_masking(nbin=self.nbin)
        self.stft = STFT(nframe, nhop, nframe)

    def forward(self, x):
        x = self.stft.transform(x)  # b,c,t,f
        batch_size, _, seq_len, _ = x.shape
        x_r_input, x_i_input = x[:, 0, :, :], x[:, 1, :, :]
        x_mag_ori, x_phase_ori = torch.norm(x, dim=1), torch.atan2(x[:, -1, :, :], x[:, 0, :, :])
        x_mag = x_mag_ori.unsqueeze(dim=1)
        ...
        x_mag_out = x_mag_mask * x_mag_ori
        x_r_out, x_i_out = (x_mag_out * torch.cos(x_phase_ori) + x_real), (
            x_mag_out * torch.sin(x_phase_ori) + x_imag
        )

        x_com_out = torch.stack((x_r_out, x_i_out), dim=1)
        out = self.stft.inverse(x_com_out)
...
class dual_aia_trans_merge_crm_fig(nn.Module):
    def __init__(self, nframe=512, nhop=256):
        ...
        self.de1 = dense_decoder(width=ch_mid, nbin=self.nbin)
        self.de2 = dense_decoder(width=ch_mid, nbin=self.nbin)
        self.de_mag_mask = dense_decoder_masking(width=ch_mid, nbin=self.nbin)
        self.stft = STFT(nframe, nhop, nframe)
    ...
    def forward(self, x, HL):
        ...
        x_mag = x_mag_ori.unsqueeze(dim=1)
        # ri/mag components enconde+ aia_transformer_merge
        x_ = torch.concat([x, hl], dim=1)
        x_ri = self.en_ri(x_)  # BCTF
        x_mag_en = self.en_mag(x_mag)
        x_last_mag, x_outputlist_mag, x_last_ri, x_outputlist_ri = self.aia_trans_merge(
            x_mag_en, x_ri
        )  # BCTF, #BCTFG
    ...
    def loss(self, lbl, enh):
        xk_lbl = self.stft.transform(lbl)
        xk_enh = self.stft.transform(enh)
        mag_lbl = xk_lbl.pow(2).sum(1).sqrt()
        mag_enh = xk_enh.pow(2).sum(1).sqrt()

        loss_mag = torch.square(mag_lbl - mag_enh).mean()
        loss_ri = torch.square(xk_lbl - xk_enh).mean()

        loss = loss_mag + loss_ri
        return dict(loss=loss, mag_lv=loss_mag.detach(), ri_lv=loss_ri.detach())
