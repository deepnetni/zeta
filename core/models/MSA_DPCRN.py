...
class MSA_DPCRN_SPEC_onnx(nn.Module):
    ...
    def forward(self, specs_mic, specs_ref, *state):
        ...
        return spec_enh, x, self.unpack_state(state)
...
def verify_spec():
    ...
    with torch.no_grad():
        out = net(mic, ref)
        out_spec, x = net_spec(xk_mic, xk_ref)
    out_spec = stft.inverse(out_spec)
    out_spec = torch.clamp(out_spec, -1, 1)

    print(out.shape, out_spec.shape, x.shape, ((out - out_spec) ** 2).sum())
...
def verify_online():
    ...
    for nt in range(xk_mic.size(2)):
        ...
        with torch.no_grad():
            out_f, _, state = net_spec(mic_frame, ref_frame, state)
