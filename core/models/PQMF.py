...
class PQMF(torch.nn.Module):
    ...
    def __init__(self, subbands=4):
        ...
        self.taps = int(
            (self.A - 8) / (2.285 * (0.8 / self.subbands) * np.pi)
        )  # (0.8/subbands * pi) is the width of band-transition
        if self.taps % 2 != 0:
            self.taps += 1
        ...
        for k in range(self.subbands):
            updown_filter[
                k, k, 0
            ] = 1.0  # only the 1st kernel, i.e., zero to the other right samples
...
if __name__ == "__main__":
    ...
    qmf = PQMF(2)
    data, fs = sf.read("sph.wav")
    data = torch.from_numpy(data)[None, :].float()  # B,T
    ret = qmf.analysis(data)  # sub, T//sub
    a, b = ret.chunk(2, dim=1)
    a.squeeze_(1)
    print(ret.shape, a.shape, b.shape)

    # sf.write("l.wav", ret[0], int(fs // 2))
    # sf.write("h.wav", ret[1], int(fs // 2))

    syn = qmf.synthesis(ret)  # B,1,T
    print(syn.shape)
    print(syn.shape, torch.mean((syn - data) ** 2))
    sf.write("r.wav", syn[0][0], fs)
    # cdata, fs = sf.read("out.wav")
    # L = len(cdata)
    # cdata = torch.from_numpy(cdata)[None, :].float()  # B,T
    # print(torch.mean((cdata - data[..., :L]) ** 2))
