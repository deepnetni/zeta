...
def create_wham(
    wsj_root, wham_noise_path, output_root, wsjmix_16k_root=None, wsjmix_8k_root=None
):
            ...
            for datalen_dir in ["max", "min"]:
                ...
                for i_utt, output_name in enumerate(utt_ids):
                    if from_scratch:
                        ...
                        print("#", s1_path, s2_path)
                        s1 = read_scaled_wav(
                            s1_path, scaling_wsjmix[i_utt][0], downsample
                        )
                        s1 = quantize(s1) * scaling_speech[i_utt]
                        s2 = read_scaled_wav(
                            s2_path, scaling_wsjmix[i_utt][1], downsample
                        )
                    ...
                    for dir, samp in zip(dirs, samps):
                        sf.write(
                            os.path.join(output_path, dir, output_name),
                            samp,
                            sr,
                            subtype="FLOAT",
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wsj0-root", type=str, help="Path to the folder containing wsj0/"
    )
    parser.add_argument(
        "--wham-noise-root",
        type=str,
