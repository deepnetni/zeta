    ...
    for splt in SPLITS:
        ...
        for wav_dir in ["wav" + sr for sr in SAMPLE_RATES]:
            for datalen_dir in DATA_LEN:
                ...
                for sfx in SUFFIXES:
                    os.makedirs(
                        os.path.join(output_path, CLEAN_DIR + sfx), exist_ok=True
                    )
                    os.makedirs(
                        os.path.join(output_path, SINGLE_DIR + sfx), exist_ok=True
                    )
                    os.makedirs(
                        os.path.join(output_path, BOTH_DIR + sfx), exist_ok=True
                    )
        ...
        for i_utt, output_name in enumerate(utt_ids):
            ...
            noise_samples_temp = read_scaled_wav(
                os.path.join(noise_path, output_name), 1
            )
            s1_temp, s2_temp, noise_samples_temp = append_or_truncate(
                s1_temp, s2_temp, noise_samples_temp, "max", start_samp_16k=0
            )  # don't pad beginning yet
            ...
            for sr_i, sr_dir in enumerate(SAMPLE_RATES):
                ...
                for datalen_dir in DATA_LEN:
                    ...
                    wham_speech_key = "scaling_wham_speech_{}_{}".format(
                        sr_dir, datalen_dir
                    )
                    wham_noise_key = "scaling_wham_noise_{}_{}".format(
                        sr_dir, datalen_dir
                    )
                    ...
                    s1 = read_scaled_wav(
                        s1_path, scaling_npz[wsjmix_key][i_utt][0], downsample
                    )
                    s1 = quantize(s1) * scaling_npz[wham_speech_key][i_utt]
                    s2 = read_scaled_wav(
                        s2_path, scaling_npz[wsjmix_key][i_utt][1], downsample
                    )
                    ...
                    s1_spatial_scaling = np.sqrt(
                        np.sum(s1**2) / np.sum(anechoic[sr_i][0, LEFT_CH_IND, :] ** 2)
                    )
                    s2_spatial_scaling = np.sqrt(
                        np.sum(s2**2) / np.sum(anechoic[sr_i][1, LEFT_CH_IND, :] ** 2)
                    )

                    noise_samples_full = read_scaled_wav(
                        os.path.join(noise_path, output_name),
                    ...
                    s1_anechoic, s2_anechoic = fix_length(
                        anechoic[sr_i][0, ch_ind, :out_len].T * s1_spatial_scaling,
                        anechoic[sr_i][1, ch_ind, :out_len].T * s2_spatial_scaling,
                        datalen_dir,
                    )
                    s1_reverb, s2_reverb = fix_length(
                        reverberant[sr_i][0, ch_ind, :out_len].T * s1_spatial_scaling,
                    ...
                    for i_sfx, (sfx, source_pair) in enumerate(zip(SUFFIXES, sources)):
                        s1_samples, s2_samples, noise_samples = append_or_truncate(
                            source_pair[0],
                            source_pair[1],
                            noise_samples_full,
                            datalen_dir,
                            start_samp_16k[i_utt],
                            downsample,
                        )

                        mix_clean, mix_single, mix_both = create_wham_mixes(
                            s1_samples, s2_samples, noise_samples
                        )
                        ...
                        for dir, samp in zip(dirs, samps):
                            sf.write(
                                os.path.join(output_path, dir + sfx, output_name),
                                samp,
                                sr,
                                subtype="FLOAT",
                            )
                        ...
                        if (
                            i_sfx == 0
                        ):  # only write noise once as it doesn't change between anechoic and reverberant
                            sf.write(
                                os.path.join(output_path, NOISE_DIR, output_name),
                                noise_samples,
                                sr,
                                subtype="FLOAT",
                            )
