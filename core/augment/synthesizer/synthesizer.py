...
def audioread(path, sr=None, norm=False, dtype=np.float64, resample_type: str = "kaiser_fast"):
    ...
    if len(x.shape) == 1:  # mono
        ...
        # x is T,C
        if sr and (xsr != sr):
            x = librosa.resample(x, orig_sr=xsr, target_sr=sr, res_type=resample_type, axis=0)
            xsr = sr
...
class DatasetDict:
    def __init__(self, config, sample_rate, resample_type):
        ...
        for dataset_name, data_config in config.items():
            ...
            # print(files) if dataset_name == "noise_92" else None
            if len(files) > 0:
    ...
    def sample(self, duration, normalize=False, thres=-35):
        ...
        while True:
            ...
            if check_power(data["audio"], threshold=thres):
                break
            # else:
            #     print(f"{meta['files']}")
...
class Synthesizer:
    ...
    def _generate_nearend(self):
        ...
        if random.random() < self.cfg["onlinesynth_nearend_prop_noisy"]:
            ...
            x_nearend, x_noise, norm_scaler = self._mix_signals(
                x_nearend, x_noise, snr, rms_clean=rms(x_target), rms_noise=rms(x_noise)
            )
            x_target /= norm_scaler
