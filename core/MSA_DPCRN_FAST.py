...
from core.models.complexnn import (
    ComplexConv2d,
...
from core.models.ft_lstm import FTLSTM_RESNET
from core.models.Fusion.ms_cam import MS_CAM_F
...
class MSA_DPCRN_ALIGN(nn.Module):
    def __init__(
        self,
        nframe: int,
        nhop: int,
    ...
    def forward(self, mic, ref):
        ...
        x_r = torch.concat([x_r, torch.zeros(*x_r.shape[:-1], 1).to(x.device)], dim=-1)
        x_i = torch.concat([x_i, torch.zeros(*x_i.shape[:-1], 1).to(x.device)], dim=-1)
...
class MSA_DPCRN_SPEC_ALIGN(nn.Module):
    def __init__(
        self,
        nframe: int,
        nhop: int,
...
class MSA_DPCRN_SPEC_ALIGN_online(nn.Module):
    def __init__(
        self,
        nframe: int,
        nhop: int,
...
class MSA_DPCRN_SPEC_ALIGN_onnx(nn.Module):
    def __init__(
        self,
        nframe: int,
        nhop: int,
...
def verify_onnx():
    ...
    net_spec = MSA_DPCRN_SPEC_ALIGN_onnx(
