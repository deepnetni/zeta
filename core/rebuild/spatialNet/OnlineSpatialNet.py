...
from norm import *
from retention import MultiScaleRetention, RetNetRelPos
...
class SpatialNetLayer(nn.Module):
    def __init__(
        ...
        if attention.startswith("ret"):  # e.g. ret(1,share_qk)
            ...
            self.mhsa = MultiScaleRetention(
                embed_dim=dim_hidden,
                num_heads=num_heads,
                value_factor=value_factor,
                share_qk=attn_params[1] == "share_qk",
            )
            ...
            self.mhsa = MultiheadAttention(
                embed_dim=dim_hidden, num_heads=num_heads, batch_first=True
            )
    ...
    def forward(
        ...
        if isinstance(self.mhsa, Mamba):
            ...
            x_, attn = self._tsa(
                x, att_mask, chunkwise_recurrent, rope, state=state, inference=inference
            )
            x = x + x_
        if isinstance(self.tconvffn, Mamba):
            x = x + self._mamba(
                x, self.tconvffn, self.norm_tconvffn, self.dropout_tconvffn, inference
            )
        else:
    ...
    def _tsa(
        ...
        if isinstance(self.mhsa, MultiheadAttention):
            ...
            if inference == False:
                ...
                xs, state = [], dict()
                for i in range(T):
                    xi = self.mhsa.forward(
                        x[:, [i], :], rel_pos=attn_mask[i], incremental_state=state
                    )
                    xs.append(xi)
                x = torch.concat(xs, dim=1)
            attn = None
    ...
    def _tconvffn(self, x: Tensor, state: Dict[int, Any] = None) -> Tensor:
        ...
        for m in self.tconvffn:
            if isinstance(m, CausalConv1d):
                ...
                isinstance(m, nn.GroupNorm) or "GroupNorm" in type(m).__name__
            ):  # normalize along H & F
...
class OnlineSpatialNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert (
            attention.startswith("mhsa")
            or attention.startswith("ret")
            or attention.startswith("mamba")
        ), attention
        assert (
            attention.startswith("mhsa")
            or attention.startswith("mamba")
    ...
    def forward(
        self, x: Tensor, inference: bool = False, return_attn_score: bool = False
    ) -> Tensor:
    ...
    def get_causal_mask(
        ...
        if isinstance(self.pos, RetNetRelPos):
            ...
            pos1 = torch.arange(
                start=0, end=slen, dtype=torch.long, device=device, requires_grad=False
            ).unsqueeze(1)
            pos2 = torch.arange(
                start=0, end=slen, dtype=torch.long, device=device, requires_grad=False
            ).unsqueeze(0)
            ...
            if self.rope == "ALiBi":
                ...
                m = (2.0 ** (-8 / torch.arange(1, self.num_heads + 1, 1, device=device))).reshape(
                    self.num_heads, 1, 1
                )
                m = torch.concat([m] * batch_size, dim=0)
                relative_pos = torch.where(
                    (relative_pos >= 0) * (relative_pos < self.attn_scope),
                    relative_pos.abs() * -1,
                    -torch.inf,
                )
                mask = m * relative_pos
            ...
            mask = torch.where(
                (relative_pos >= 0) * (relative_pos < self.attn_scope), 0.0, -torch.inf
            )
        return mask
