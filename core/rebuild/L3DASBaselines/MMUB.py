...
class MIMO_UNet_Beamforming(nn.Module):
    def __init__(
        ...
        self.decoder_inchannel = list(
            map(lambda x: x[0] + x[1], zip(unet_channel[1:] + [0], unet_channel))
        )
    ...
    def forward(self, inputs, device):
        ...
        est_speech_stft = torch.cat(
            (est_speech_stft, torch.zeros(batch_size, 1, frame).to(device)), 1
        )
...
if __name__ == "__main__":
    ...
    inputs = torch.rand(batch_size, audio_channel, length)  # B,C,T
    ...
    with warnings.catch_warnings():
        ...
        flops, params = profile(model, inputs=(inputs, "cpu"), verbose=False)

    print(f"FLOPs={flops / 4.792/1e9}, params={params/1e6:.2f}")
