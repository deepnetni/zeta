...
def Sp_and_phase(signal):
    ...
    F = librosa.stft(
        y_pad, n_fft=400, hop_length=100, win_length=400, window=scipy.signal.windows.hamming(400)
    )
...
if __name__ == "__main__":
    ...
    wave, fs = audioread("/home/deepni/datasets/dns_wdrc/libri_demand_test/0_enlarge_nearend.wav")
    out = cal_pcs(wave)

    out = np.stack([wave, out], axis=-1)
    audiowrite("~/out2.wav", out, fs)
