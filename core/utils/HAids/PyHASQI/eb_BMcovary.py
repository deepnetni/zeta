...
def eb_BMcovary(xBM, yBM, segsize, fsamp):
    ...
    halfcorr = 1. / crosscorrelation(halfwindow, halfwindow, maxlag)
    halfsum2 = 1.0 / np.sum(halfwindow ** 2)
    ...
    for k in range(nchan):
        for i in range(frame_num):
            if i == 0:
                ...
                Mxy_np = np.correlate(segx, segy, mode='full')[len(segy)-maxlag-1:len(segy)+maxlag]
                Mxy_fft = scipy.signal.fftconvolve(segx, segy[::-1], mode='full')[len(segy)-maxlag-1:len(segy)+maxlag]
...
def eb_BMcovary_new(xBM, yBM, segsize, fsamp):
    ...
    for i in range(frame_num):
        ...
        if i == 0:
            ...
            head_Mxy = np.max(np.abs(scipy.signal.fftconvolve(head_segx, head_segy[:, ::-1], mode='full', axes=1)[:, head_segx.shape[1]-maxlag-1:head_segx.shape[1]+maxlag] * halfcorr), axis=1)
            ...
            tail_Mxy = np.max(np.abs(scipy.signal.fftconvolve(tail_segx, tail_segy[:, ::-1], mode='full', axes=1)[:, tail_segx.shape[1]-maxlag-1:tail_segx.shape[1]+maxlag] * halfcorr), axis=1)
            ...
            Mxy = np.max(np.abs(scipy.signal.fftconvolve(segx, segy[:, ::-1], mode='full', axes=1)[:, segx.shape[1]-maxlag-1:segx.shape[1]+maxlag] * wincorr), axis=1)
