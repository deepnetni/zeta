...
class Freq_Conformer(nn.Module):
    def __init__(self, dim_model, num_head, dropout):
        ...
        self.self_attn = nn.MultiheadAttention(
            embed_dim=dim_model, num_heads=num_head, dropout=dropout, batch_first=True
        )
...
class proposed_block(nn.Module):
    def __init__(self, ch_dim, num_head, dropout, depth):
        ...
        self.freq_conformer = Freq_Conformer(
            dim_model=ch_dim, num_head=num_head, dropout=dropout
        )
        self.temp_conformer = Time_Conformer(
            dim_model=ch_dim, num_head=num_head, dropout=dropout, num_layers=3
        )
...
class Network(nn.Module):
    def __init__(
        ...
        self.inp_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=2 * self.mic_num,
                out_channels=self.ch_dim,
                kernel_size=(5, 5),
                padding=(2, 2),
            ),
            nn.LayerNorm(257),
        ...
        for ii in range(num_layer):
            self.proposed_block.append(
                proposed_block(
                    ch_dim=ch_dim, num_head=num_head, dropout=dropout, depth=depth
                )
            )

        self.out_conv = nn.Conv2d(
            in_channels=self.ch_dim, out_channels=2, kernel_size=(5, 5), padding=(2, 2)
        )
    ...
    def pad_signal(self, input):
        ...
        pad_aux = Variable(torch.zeros(batch_size, nchannel, self.hop)).type(
            input.type()
        )
    ...
    def forward(self, x):
        ...
        xi = rearrange(xk, "(b m) c t f->b (m c) t f", b=nB)
        ...
        #     istft_input, n_fft=self.win, hop_length=self.hop, return_complex=False
        # )

        # feat = masked_enc_out # B,2,T,F
        feat = mask # B,2,T,F

        output = self.stft.inverse(feat)  # B, T
