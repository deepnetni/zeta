...
from utils.losses import loss_compressed_mag, loss_pmsqe, loss_sisnr, loss_snr
from utils.record import REC, RECDepot
from utils.stft_loss import MultiResolutionSTFTLoss
from utils.trunk import CHiMe3, SpatialedDNS, pad_to_longest
...
class Train(Engine):
    ...
    def loss_fn(self, clean: Tensor, enh: Tensor, nlen=None) -> Dict:
        ...
        # loss_pase = F.mse_loss(clean_pase.flatten(-1), enh_pase.flatten(-1))
        ...
        snr_loss = loss_snr(clean, enh)
        sisnr_loss = loss_sisnr(clean, enh)
        ...
        # loss = 0.05 * sisnr_lv + mse_pha + mse_mag + 0.3 * pmsq_score
        # return {
        #     "loss": loss,
        ...
        # loss = sc_loss + mag_loss  # + 0.3 * pmsqe_score  # + loss_pase  # + 0.05 * sdr_lv
        # loss = loss + 0.3 * sisnr_lv
        # loss = loss + 0.3 * snr_loss
        # loss = 0.3 * pmsqe_loss
        # loss = mag_loss + sc_loss

        return {
            "loss": loss,
            "sc": sc_loss.detach(),
            "mag": mag_loss.detach(),
            "pmsqe": 0.3 * pmsqe_loss.detach(),
            "sisnr": 0.3 * sisnr_loss.detach(),
            "snr": 0.3 * snr_loss.detach(),
            # "pase": loss_pase.detach(),
    ...
    def prediction_per_epoch(self, epoch):
        ...
        idx = 0
        for mic, sph, nlen in self.vtest_loader:
            if idx >= 20:
                break
            ...
            for i in range(nB):
                audiowrite(f"{outdir}/{idx}_enh.wav", enh[i].cpu().numpy(), self.fs)
                audiowrite(f"{outdir}/{idx}_mic.wav", mic[i].cpu().numpy(), self.fs)
                audiowrite(f"{outdir}/{idx}_sph.wav", sph[i].cpu().numpy(), self.fs)
                idx += 1
...
def parse():
    ...
    parser.add_argument("--basedir", help="train mode", default=None)
...
if __name__ == "__main__":
    ...
    # cfg_fname = "config/config_mcse_skip_exp.ini" if args.conf is None else args.conf
    ...
    cfg["config"]["epochs"] = cfg["config"]["epochs"] if args.epoch is None else args.epoch
    ...
    if args.train:
        ...
        train_dset = CHiMe3(cfg["chime"]["train_dset"], subdir="train", nlen=3.0, min_len=1.0)
        valid_dset = CHiMe3(cfg["chime"]["valid_dset"], subdir="dev")
        test_dsets = CHiMe3(cfg["chime"]["vtest_dset"], subdir="test")
        ...
        if args.valid:
            ...
            dset = CHiMe3(cfg["chime"]["vtest_dset"], subdir="test")
        else:
