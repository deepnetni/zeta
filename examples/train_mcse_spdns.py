...
class Train(Engine):
    ...
    def loss_fn(self, clean: Tensor, enh: Tensor, nlen=None) -> Dict:
        ...
        # }
        # sisnr_lv = 0.05 * loss_sisnr(clean, enh)
...
if __name__ == "__main__":
    ...
    if args.train:
        cfg["config"]["info_dir"] = f'{cfg["config"]["info_dir"]}_spdns'

        train_dset = SpatialedDNS(
            dirname=cfg["spdns"]["train_dset"],
            patten="**/*mic.wav",
            flist="spdns_train.csv",
            keymap=("mic", "target"),
            nlen=3.0,
            min_len=1.0,
        )
        valid_dset = SpatialedDNS(
            dirname=cfg["spdns"]["valid_dset"],
            flist="spdns_valid.csv",
            patten="**/*mic.wav",
            keymap=("mic", "target"),
        )
        test_dsets = SpatialedDNS(
            dirname=cfg["spdns"]["vtest_dset"],
            flist="spdns_test.csv",
            patten="**/*mic.wav",
            keymap=("mic", "target"),
        )
