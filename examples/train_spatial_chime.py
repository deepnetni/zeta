...
from rebuild.spatialNet.SpatialNet import SpatialNet
from rebuild.spatialNet.norm import Norm

# from rebuild.spatialNet.OnlineSpatialNet import OnlineSpatialNet
...
class Train(Engine):
    def __init__(
        ...
        **kwargs,
        ...
        self.stft.eval()
        self.inp_norm = Norm(mode="frequency", online=False).cuda()
    ...
    def _fit_each_epoch(self, epoch):
        ...
        for mic, sph in pbar:
            ...
            mic = einops.rearrange(mic, "b t c-> (b c) t")
            xk_mic = self.stft.transform(mic)
            xk_mic = einops.rearrange(xk_mic, "(b m) c t f->b f t m c", b=nB).contiguous()
            xk_mic = torch.view_as_complex(xk_mic)  # b f t m
            xk, (xk_r, xk_rmm) = self.inp_norm.norm(xk_mic, ref_channel=0)
            xk = torch.view_as_real(xk).flatten(-2)  # b,f,t,(m 2) -> b,f,t,m2
            xk_enh = self.net(xk)  # b f t m2
            xk_enh = einops.rearrange(xk_enh, "b f t (m c)->b f t m c", c=2).contiguous()
            xk_enh = torch.view_as_complex(xk_enh)  # b f t 1
            xk_enh = self.inp_norm.inorm(xk_enh, (xk_r, xk_rmm))
            xk_enh = torch.view_as_real(xk_enh)  # b f t 1 2
            xk_enh = einops.rearrange(xk_enh, "b f t m c-> b (m c) t f")
            enh = self.stft.inverse(xk_enh)
    ...
    def valid_fn(self, sph: Tensor, enh: Tensor, nlen_list: List, return_loss: bool = True) -> Dict:
        """
        B,T
        """
        # sisnr_l = []
    ...
    def _net_flops(self) -> int:
        ...
        xk = self.stft.cpu().transform(x)
        xk = einops.rearrange(xk, "(b m) c t f->b f t (c m)", b=1)
...
def parse():
    ...
    parser.add_argument("--test", help="fusion with the atten method", action="store_true")
    ...
    parser.add_argument("--valid-first", help="valid first", action="store_true")
...
if __name__ == "__main__":
    ...
    cfg_fname = "config/config_spatial.ini"
    cfg = read_ini(cfg_fname)
    print("##", cfg_fname)

    # net = OnlineSpatialNet(
    #     dim_input=12,
    #     dim_output=4,
    #     num_layers=8,
    #     dim_hidden=96,
    ...
    if args.train:
        cfg["config"]["info_dir"] = f'{cfg["config"]["info_dir"]}_chime3'
        train_dset = CHiMe3(cfg["dataset"]["train_dset"], subdir="train", nlen=3.0, min_len=1.0)
        ...
        net.cuda()
        net.eval()
        stft = STFT(nframe=512, nhop=256).cuda()
        stft.eval()
        inp_norm = Norm(mode="frequency", online=False).cuda()

        if args.valid:
            dset = CHiMe3(cfg["dataset"]["valid_dset"], subdir="dev")
        elif args.vtest:
            dset = CHiMe3(cfg["dataset"]["vtest_dset"], subdir="test")
        else:
        ...
        for d, fname in tqdm(dset):
            d = d.cuda()
            nB = d.size(0)
            d = einops.rearrange(d, "b t c-> (b c) t")
            xk_mic = stft.transform(d)
            xk_mic = einops.rearrange(xk_mic, "(b m) c t f->b f t m c", b=nB).contiguous()
            xk_mic = torch.view_as_complex(xk_mic)  # b f t m
            xk, (xk_r, xk_rmm) = inp_norm.norm(xk_mic, ref_channel=0)
            xk = torch.view_as_real(xk).flatten(-2)  # b,f,t,(m 2) -> b,f,t,m2
            with torch.no_grad():
                xk_enh = net(xk)
            xk_enh = einops.rearrange(xk_enh, "b f t (m c)->b f t m c", c=2).contiguous()
            xk_enh = torch.view_as_complex(xk_enh)  # b f t 1
            xk_enh = inp_norm.inorm(xk_enh, (xk_r, xk_rmm))
            xk_enh = torch.view_as_real(xk_enh)  # b f t 1 2
            xk_enh = einops.rearrange(xk_enh, "b f t m c-> b (m c) t f")
            enh = stft.inverse(xk_enh)
            out = enh.cpu().detach().squeeze().numpy()
