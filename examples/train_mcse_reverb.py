...
from utils.trunk import CHiMe3, SpatialedDNS, pad_to_longest, L3DAS22, ReverbChallenge
...
from models.mcse_skip_df_exp import aia_mcse_skip_sd_df
...
class Train(Engine):
    def __init__(
        self,
        train_dset: Dataset,
        valid_dset: Union[Dict[str, Dataset], Dataset],
        vtest_dset: Union[Dict[str, Dataset], Dataset],
        ...
        **kwargs,
        ...
        if isinstance(valid_dset, Dict):
            self.valid_loader = {
                ...
                for n, d in valid_dset.items()
            }
            self.valid_dset = valid_dset
        else:
            self.valid_loader = {
                valid_dset.dirname: DataLoader(
                    valid_dset,
                    batch_size=2,
                    # batch_size=1,
                    num_workers=4,
            ...
            # self.valid_dset = dict(v=valid_dset)

        if isinstance(vtest_dset, Dict):
            self.vtest_loader = {
                n: DataLoader(
                    d,
                    batch_size=2,
                ...
                for n, d in vtest_dset.items()
            }
            self.vtest_dset = vtest_dset
        else:
            self.vtest_loader = {
                vtest_dset.dirname: DataLoader(
                    vtest_dset,
                    batch_size=2,
                    # batch_size=1,
                    num_workers=4,
            ...
            # self.vtest_dset = dict(t=vtest_dset)
    ...
    def _valid_dsets(self):
        ...
        # -----------------------#
        metric_rec = REC()
        for n, dset in self.valid_loader.items():
            pbar = tqdm(
                dset,
                ncols=120,
                leave=False,
            ...
            dset_dict[f"{n}_valid"] = metric_rec.state_dict()

        # -----------------------#
        ##### vtest dataset ######
        # -----------------------#
        metric_rec = REC()
        for n, dset in self.vtest_loader.items():
            pbar = tqdm(
                dset,
                ncols=120,
                leave=False,
                # desc=f"v-{self.vtest_dset[n].dirname}",
                desc=f"t-{n}",
            )
            ...
            dset_dict[f"{n}_vtest"] = metric_rec.state_dict()
    ...
    def _valid_each_epoch(self, epoch):
        ...
        for n, dset in self.valid_loader.items():
            ...
            for k, v in metric_rec.state_dict().items():
                if k in self.raw_metrics[f"{n}_valid"]:
                    out[f"{n}_{k}"] = {"raw": self.raw_metrics[f"{n}_valid"][k], "enh": v}
                else:
                    out[f"{n}_{k}"] = v
    ...
    def _vtest_each_epoch(self, epoch):
        ...
        for n, dset in self.vtest_loader.items():
            ...
            dirn = {}
            for k, v in metric_rec.state_dict().items():
                if k in self.raw_metrics[f"{n}_vtest"]:
                    dirn[k] = {"raw": self.raw_metrics[f"{n}_vtest"][k], "enh": v}
                else:
                    dirn[k] = v
...
def parse():
    ...
    parser.add_argument("--df", help="input directory", action="store_true")
...
if __name__ == "__main__":
    ...
    # ch must be multiply of 4,6  ; 12
    if args.df:
        cfg_fname = "config/config_mcse_skip_exp+df.ini"
        # net = aia_mcse_skip_sd_df(in_channels=6, feature_size=257, mid_channels=72)
    else:
        cfg_fname = "config/config_mcse_skip_exp.ini"
        # net = aia_mcse_skip_sd(in_channels=6, feature_size=257, mid_channels=72)
        # net = aia_mcse_skip_sd_upch(in_channels=6, feature_size=257, mid_channels=72)
        # net = aia_mcse_skip_sd_upch_att(
        #     in_channels=6, feature_size=257, mid_channels=72
        # )
        #
    # md_conf = {"in_channels": 8, "feature_size": 257, "mid_channels": 72}
    md_conf = {"in_channels": 8, "feature_size": 257, "mid_channels": 96}
    ...
    if args.pred is False:
        cfg["config"]["info_dir"] = f'{cfg["config"]["info_dir"]}_reverb'
        train_dset = ReverbChallenge(
            dirname=cfg["reverb"]["train_dset"],
            dclass="train",
            flist="reverb_tr.csv",
            min_len=1.0,
            nlen=3.0,
        ...
        valid_dset = {
            t: ReverbChallenge(
                dirname=cfg["reverb"]["valid_dset"],
                dclass=t,
                flist=f"reverb_dt_{t}.csv",
            )
            for t in ("far", "near")
        }
        test_dsets = {
            t: ReverbChallenge(
                dirname=cfg["reverb"]["vtest_dset"],
                dclass=t,
                flist=f"reverb_et_{t}.csv",
            )
            for t in ("far", "near")
        }
