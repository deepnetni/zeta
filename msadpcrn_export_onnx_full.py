...
from models.MSA_DPCRN import MSA_DPCRN_SPEC, MSA_DPCRN_SPEC_onnx, MSA_DPCRN
...
def to_onnx():
    ...
    h = [
        torch.zeros(4, 1, 65, 64),  # 65 is the freq, 64 is the hidden size
        torch.zeros(4, 1, 65, 64),
    ]
...
def inference():
    ...
    out_list = []
    state = [
        np.zeros([4, 1, 65, 64], dtype=np.float),
        np.zeros([4, 1, 65, 64], dtype=np.float),
    ]
