...
ckpt = r"E:\model_results_trunk\AEC\trained_msadpcrn_align\msa_dpcrn_hann_sqrt_win_phase_loss_16_32_32_64_2211_48\checkpoints\epoch_0086.pth"
...
def to_onnx():
    ...
    h = [
        torch.zeros(4, 1, 64, 48).float(),  # 65 is the freq, 64 is the hidden size
        torch.zeros(4, 1, 64, 48).float(),
    ]
...
def inference():
    ...
    # mic, _ = audioread("aec_test/mic_3.wav")
    # ref, _ = audioread("aec_test/ref_3.wav")
    # mic, _ = audioread("record_input.wav")
    # ref, _ = audioread("record_output.wav")
    mic, _ = audioread("aec_test/record_input.wav")
    ref, _ = audioread("aec_test/record_output.wav")
    ...
    N = min(len(mic), len(ref))
    mic = torch.from_numpy(mic).float()[None, :N]
    ref = torch.from_numpy(ref).float()[None, :N]
    mic = stft.transform(mic)
    ref = stft.transform(ref)
    # print(mic.shape)
    out_list = []
    state = [
        np.zeros([4, 1, 64, 48], dtype=np.float32),
        np.zeros([4, 1, 64, 48], dtype=np.float32),
    ]
    ...
    out = out.squeeze().numpy()
    audiowrite(f"aec_test/onnx_out_{align}.wav", out, 16000)
    return out
